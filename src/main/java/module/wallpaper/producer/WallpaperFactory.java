...
public class WallpaperFactory {
    ...
    public BufferedImage buildWallpaper(URL backgroundURL, URL artURL) {
        ...
        int x = config.width - art.getWidth() - config.passepartout - config.art_padding;
        int y = config.height - art.getHeight() - config.taskbar - config.passepartout - config.art_padding;
        BufferedImage image = new PainterBackground(ctx).paint(
                wallpaper,
                painter.getSize().height + config.passepartout * 2,
                art.getWidth() + config.art_padding, art.getHeight() + config.art_padding
        );
    ...
    private BufferedImage paintCalendar(BufferedImage image, PainterCalendar painter, int index, Context ctx) {
        int margin = Config.getInstance().passepartout;
        return painter.paint(
            image,
        image.getWidth() - painter.getSize().width * index - ( index==0 ? 0 : margin/2*index) - margin/2,
            margin,
            ctx.getMonth(),
            ctx.getYear()
        );
